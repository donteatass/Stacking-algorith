...
class LiftPlannerV1P5:
        ...
        for flag in last_to_first_flags[1:]:
            if flag == run_type:
                ...
                if run_type:
                    ...
                    # Blocker run
                    if location_complete and stack.space_left() >= run_len:
        ...
        if total <= self.hand_capacity:
            ...
            if blockers == 0 or self.temp.space_left() >= blockers:
                ...
                if blockers > 0:
                    ...
                    if not has_deeper_targets and stack.space_left() >= blockers:
    ...
    def plan_mode_B_old_version1(self, count: int = 28):
        self._mode_B_active = True
        selected = 0
        while selected < count:
            # 1) Top-run of cleared? Take as many as possible in one pick.
            runs = []
            for s in self.sources:
                run_len = self._contiguous_top_cleared(s)
                if run_len > 0:
                    runs.append((min(run_len, self.hand_capacity, count - selected), run_len, s))
            if runs:
                runs.sort(key=lambda t: (-t[0], -t[1]))
                take_now, run_len, stack = runs[0]
                self._pick(stack, take_now, note=f"Pick {take_now} cleared from top (Mode B batched)")
                self._drop(self.dest, take_now, note=f"Deliver {take_now} cleared to DEST (Mode B batched)")
                selected += take_now
                self.maybe_early_temp_return()
                continue

            # 2) Otherwise, use one-pick multi-seg starting at first cleared under blockers.
            candidates = []
            for s in self.sources:
                for idx, sat in enumerate(s.items):
                    if sat.cleared and sat.sat not in {x.sat for x in self.dest.items}:
                        candidates.append((idx, s, sat))
                        break
            if not candidates:
                break
            def deeper_cleared(s: 'StackState', i: int) -> int:
                return sum(1 for x in s.items[i+1:] if x.cleared)
            candidates.sort(key=lambda t: (t[0], -deeper_cleared(t[1], t[0])))
            idx, stack, sat = candidates[0]

            def is_cleared(x: Sat) -> bool: return x.cleared and x.sat not in {y.sat for y in self.dest.items}
            remaining_limit = count - selected
            if self._one_pick_multiseg(stack, is_cleared, remaining_limit, note_prefix="Mode B"):
                # Count how many we actually delivered in that one pick (drop log's last segments include it)
                # Simpler: recompute from the items we just picked (which are last K in self.dest / or count before/after).
                delivered_now = 0
                # We can count by scanning last actions since last pick until blockers stash is complete;
                # easier: recompute from last picked slice we just removed: it's the items we moved to either dest or temp.
                # Here we conservatively update 'selected' by min(remaining_limit, number of cleared sats now in Dest compared to before).
                selected += min(remaining_limit, sum(1 for x in self.dest.items[-self.hand_capacity:] if True))  # dummy safe increment
                # But that approximation may overcount; instead, recompute selected from Dest contents:
                selected = len([x for x in self.dest.items if True])  # In Mode B, every Dest item is cleared.
                continue

            # 3) Fallback to single-target extraction (one-lift/peel)
            self.target_ids = {sat.sat}
            self._extract_one_from_stack(stack)
            self.target_ids = set()
            selected = len([x for x in self.dest.items])  # update from dest
        self._mode_B_active = False
        self.return_all_temp()
    ...
    def plan_mode_B_old_version2(self, count: int = 28):
        """
        Execute Mode B: deliver up to `count` cleared satellites with minimal lifts.

        On each iteration, the planner considers two types of actions:

        1. **Cross-stack top-run pick**: collect contiguous cleared satellites from the
           tops of multiple stacks (no blockers). This may deliver more than the
           remaining target count but is allowed to minimize picks.

        2. **Single-stack multi-segment pick**: within a single stack, take a
           prefix that may include blockers to reach deeper cleared satellites.
           The candidate prefix is chosen to maximize the number of deliverables,
           with a secondary preference for fewer blockers.

        The planner selects whichever option yields more deliverables. If neither
        yields any deliverables, it falls back to the existing multi-segment or
        single-target extraction logic.
        """
        self._mode_B_active = True
        selected = 0
        while selected < count:
            remaining_target = count - selected
            # Build cross-stack deliver-run option
            deliver_candidates: List[Tuple[StackState, int]] = []
            for s in self.sources:
                run_len = self._contiguous_top_cleared(s)
                if run_len > 0:
                    deliver_candidates.append((s, run_len))
            deliver_candidates.sort(key=lambda t: (-t[1], t[0].name))
            # Determine picks for pure cross-run deliver-run
            cross_remaining = self.hand_capacity
            cross_picks: List[Tuple[StackState, int]] = []
            cross_deliver_count = 0
            for s, rlen in deliver_candidates:
                if cross_remaining <= 0:
                    break
                k = min(rlen, cross_remaining)
                if k > 0:
                    cross_picks.append((s, k))
                    cross_deliver_count += k
                    cross_remaining -= k
            # Build multi-segment candidates for each stack
            multi_candidates: List[Tuple[int,int,int,StackState,List[bool]]] = []
            for s in self.sources:
                max_k = min(self.hand_capacity, len(s.items))
                temp_free = self.temp.space_left()
                best_k = 0; best_del = 0; best_blk: Optional[int] = None; best_flags: List[bool] = []
                for K in range(1, max_k + 1):
                    prefix = s.items[:K]
                    flags = [x.cleared and x.sat not in {d.sat for d in self.dest.items} for x in prefix]
                    d_count = sum(1 for f in flags if f)
                    ...
                    if d_count == 0:
                        continue
                    if b_count > temp_free:
                        continue
                    if d_count > best_del or (d_count == best_del and (best_blk is None or b_count < best_blk) or (d_count == best_del and b_count == best_blk and K > best_k)):
                        best_k = K
                        best_del = d_count
                        best_blk = b_count
                        best_flags = flags
                if best_k > 0:
                    multi_candidates.append((best_del, best_blk if best_blk is not None else 0, best_k, s, best_flags))
            # Identify best multi candidate
            best_multi = None
            for cand in multi_candidates:
                d_count, b_count, k, s, flags = cand
                if best_multi is None or d_count > best_multi[0] or (d_count == best_multi[0] and b_count < best_multi[1]):
                    best_multi = cand
            # Compute combined cross-run + multi-seg option
            best_combination = None  # (total_del, blockers_count, picks, flags)
            for cand in multi_candidates:
                d_count, b_count, k, s, flags = cand
                cap_left = self.hand_capacity - k
                if cap_left <= 0:
                    total_del = d_count
                    comb_picks = [(s, k)]
                    comb_flags = flags.copy()
                else:
                    cross_picks_local: List[Tuple[StackState, int]] = []
                    cross_del_local = 0
                    cap_tmp = cap_left
                    for t, run_len in deliver_candidates:
                        if t is s:
                            continue
                        if cap_tmp <= 0:
                            break
                        take = min(run_len, cap_tmp)
                        if take > 0:
                            cross_picks_local.append((t, take))
                            cross_del_local += take
                            cap_tmp -= take
                    total_del = d_count + cross_del_local
                    comb_picks = cross_picks_local + [(s, k)]
                    comb_flags = [True] * cross_del_local + flags.copy()
                if total_del > 0:
                    if best_combination is None or total_del > best_combination[0] or (total_del == best_combination[0] and b_count < best_combination[1]):
                        best_combination = (total_del, b_count, comb_picks, comb_flags)
            # Evaluate options: cross-run alone, multi-alone (best), combination
            chosen_picks = None; chosen_flags = None; chosen_del = 0; chosen_blockers = 0; chosen_note = "Mode B"
            cross_option = None
            if cross_deliver_count > 0:
                cross_option = (cross_deliver_count, 0, cross_picks, [True]*cross_deliver_count)
            multi_option = None
            if best_multi:
                dm, bm, km, sm, flagsm = best_multi
                multi_option = (dm, bm, [(sm, km)], flagsm)
            comb_option = None
            if best_combination:
                comb_total_del, comb_blk, comb_picks, comb_flags = best_combination
                comb_option = (comb_total_del, comb_blk, comb_picks, comb_flags)
            options = []
            if cross_option:
                options.append(("cross", cross_option))
            if multi_option:
                options.append(("multi", multi_option))
            if comb_option:
                options.append(("comb", comb_option))
            best_choice = None
            for label, opt in options:
                dcnt, bcnt, pick_list, flag_list = opt
                if best_choice is None or dcnt > best_choice[1][0] or (dcnt == best_choice[1][0] and bcnt < best_choice[1][1]):
                    best_choice = (label, opt)
            if best_choice:
                label, (dcnt, bcnt, pick_list, flag_list) = best_choice
                if bcnt > 0 and self.temp.space_left() < bcnt:
                    self._ensure_temp_space(bcnt)
                self._multi_stack_pick_and_drop(pick_list, flag_list, note_prefix="Mode B")
                selected += min(dcnt, remaining_target)
                continue
            # Fallback: use one-pick multi-seg starting at first cleared under blockers
            fallback_candidates = []
            for s in self.sources:
                for idx, sat in enumerate(s.items):
                    if sat.cleared and sat.sat not in {x.sat for x in self.dest.items}:
            ...
            fallback_candidates.sort(key=lambda t: (t[0], -deeper_cleared_fallback(t[1], t[0])))
            idx, stack, sat = fallback_candidates[0]
            def is_cleared_fallback(x: Sat) -> bool: return x.cleared and x.sat not in {y.sat for y in self.dest.items}
            if self._one_pick_multiseg(stack, is_cleared_fallback, remaining_target, note_prefix="Mode B"):
                selected = len(self.dest.items)
                continue
            # Final fallback: single-target extraction
            self.target_ids = {sat.sat}
            self._extract_one_from_stack(stack)
    ...
    def _multi_stack_pick_and_drop(self, picks: List[Tuple['StackState', int]], flags: List[bool], note_prefix: str = ""):
        """Unified override: only one stack per lift and a single drop.
        All flags must be uniform (all True -> DEST, all False -> offload/TEMP).
        """
        self._ensure_unified_state()
        # Normalize picks
        picks = [(s, k) for s, k in picks if k > 0]
        if not picks:
            return
        # Enforce single stack
        stacks = {id(s) for s, _ in picks}
        if len(stacks) != 1:
            raise AssertionError("Unified mode: picks must come from a single stack.")
        stack, total = picks[0][0], sum(k for _, k in picks)
        if len(flags) != total:
            raise AssertionError("Unified mode: flags length must match items picked.")
        all_deliv = all(flags)
        none_deliv = not any(flags)
        if not (all_deliv or none_deliv):
            raise AssertionError("Unified mode: cannot mix deliverables and blockers in one lift.")
        # Perform single pick
        self._pick(stack, total, note=f"{note_prefix}: unified single-pick")
        # Choose destination
        if all_deliv:
            dest = self.dest
            note = f"{note_prefix}: unified single-drop to DEST"
            self._drop(dest, total, note=note)
        else:
            # Offload blockers to a safe source if possible; otherwise TEMP
            offload = self._choose_offload_stack(total, exclude=stack)
            if offload is None:
                if self.temp.space_left() < total:
                    self._ensure_temp_space(total)
                offload = self.temp
                note = f"{note_prefix}: unified stash to TEMP"
            else:
                note = f"{note_prefix}: unified offload to {offload.name}"
            self._drop(offload, total, note=note)

    def _one_pick_multiseg(self, stack: 'StackState', is_deliv: Callable[[Sat], bool],
                           remaining_limit: int, note_prefix: str) -> bool:
        """Unified override: choose a prefix that is *uniform* (all deliverables or all blockers),
        then perform exactly one drop to DEST (deliverables) or offload/TEMP (blockers).
        """
        # No items
        if len(stack.items) == 0:
            return False
        max_k = min(self.hand_capacity, len(stack.items))
        bestK = 0
        best_is_deliv = None
        best_deliv = 0
        # Evaluate feasible prefixes, skipping mixed ones
        for K in range(1, max_k + 1):
            prefix = stack.items[:K]
            flags = [is_deliv(s) for s in prefix]
            if any(flags) and not all(flags):
                continue  # mixed; not allowed in unified
            d = sum(1 for f in flags if f)
            if d > remaining_limit:
                continue
            # Prefer deliverable prefixes (more deliverables), otherwise blockers (smaller K)
            if d > 0:
                if d > best_deliv or (d == best_deliv and best_is_deliv is False):
                    bestK = K; best_is_deliv = True; best_deliv = d
            else:
                if best_is_deliv is None:  # consider blockers only if no deliverable prefix found yet
                    bestK = K; best_is_deliv = False; best_deliv = 0
        if bestK == 0:
            return False
        # Perform unified pick
        self._pick(stack, bestK, note=f"{note_prefix}: unified one-pick")
        if best_is_deliv:
            self._drop(self.dest, bestK, note="Unified: single-drop to DEST")
        else:
            # Offload blockers
            offload = self._choose_offload_stack(bestK, exclude=stack)
            if offload is None:
                if self.temp.space_left() < bestK:
                    self._ensure_temp_space(bestK)
                offload = self.temp
                note = "Unified: stash blockers to TEMP"
            else:
                note = f"Unified: offload blockers to {offload.name}"
            self._drop(offload, bestK, note=note)
        return True
